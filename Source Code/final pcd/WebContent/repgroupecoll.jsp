...
<div id="header-wrapper">
  ...
  <section id="hero" class="container">
    ...
    <h2>Problème <%=Integer.parseInt(request.getParameter("idprob")) %></h2>
    <br><br>
    <% 
	...
	try
	{
		...
		query = "select * from repprobcoll where idprob="+idprob;
		...
		while(res.next()){
			...
			%><div style="background: none repeat scroll 0% 0% rgba(255, 255, 255, 0.15);border-style: none solid solid;border-width: 0px 1px 1px;border: 1px solid #C2C7CC;
					...
					<br>
<%
query2 = "select * from notes where idrep="+idrep;
dbconn = new DatabaseConnection();
conn = dbconn.setConnection();
...
//{
while(res2.next()){
int idnote= res2.getInt(1);
int idnoteur = res2.getInt(3);		
int value = res2.getInt(4);	
if(idnoteur== Integer.parseInt(request.getParameter("user"))){hasnote=1;}
String noteur = fbm.getuseerbyid(idnoteur).getPseudo();
%>
Réponse notée : <%=value %> By : <%=noteur %><br>
<%
}
//}
//else {
if(hasnote==0)
{%>
<p>Veuillez noter cette réponse:</p>

	<div class="rating rating2"><!--
		--><a href="AddRateColl?idprob=<%=idprob%>&poseur=<%=poseur%>&o=<%=objectif%>&t=<%=tache%>&a=<%=artefact%>&c=<%=contrainte%>&user=<%=session.getAttribute("iduser")%>&v=5&idrep=<%=idrep %>" title="Give 5 stars">★</a><!--
		--><a href="AddRateColl?idprob=<%=idprob%>&poseur=<%=poseur%>&o=<%=objectif%>&t=<%=tache%>&a=<%=artefact%>&c=<%=contrainte%>&user=<%=session.getAttribute("iduser")%>&v=4&idrep=<%=idrep %>" title="Give 4 stars">★</a><!--
		--><a href="AddRateColl?idprob=<%=idprob%>&poseur=<%=poseur%>&o=<%=objectif%>&t=<%=tache%>&a=<%=artefact%>&c=<%=contrainte%>&user=<%=session.getAttribute("iduser")%>&v=3&idrep=<%=idrep %>" title="Give 3 stars">★</a><!--
		--><a href="AddRateColl?idprob=<%=idprob%>&poseur=<%=poseur%>&o=<%=objectif%>&t=<%=tache%>&a=<%=artefact%>&c=<%=contrainte%>&user=<%=session.getAttribute("iduser")%>&v=2&idrep=<%=idrep %>" title="Give 2 stars">★</a><!--
		--><a href="AddRateColl?idprob=<%=idprob%>&poseur=<%=poseur%>&o=<%=objectif%>&t=<%=tache%>&a=<%=artefact%>&c=<%=contrainte%>&user=<%=session.getAttribute("iduser")%>&v=1&idrep=<%=idrep %>" title="Give 1 star">★</a>
	</div>
...
<%}%>
	...
	%>
     ...
     <form method="post" action="AjoutRepColl" name="productForm" style="background: none repeat scroll 0% 0% rgba(255, 255, 255, 0.15);border-style: none solid solid;border-width: 0px 1px 1px;border: 1px solid #C2C7CC;
border-color: -moz-use-text-color rgba(255, 255, 255, 0.5) rgba(255, 255, 255, 0.5);color: #FFF;display: inline-block;transition: color 0.25s ease-in-out 0s, border-color 0.25s ease-in-out 0s, background-color 0.25s ease-in-out 0s;
text-decoration: none;
border-radius: 0.35em;
padding: 0.8em 2em;box-sizing: border-box;">
    ...
    <input type="hidden" name="i" id="i" value="<%=idprob%>">
