  ...
  <section id="hero" class="container">
    ...
    %>
      <h2>Bienvenue dans votre groupe de travail <br>
        Vous pouvez ici échanger toutes vos idées</h2>
        <%
        }
        else
        {%><h2>Bienvenue dans ce groupe  <br>
        Vous pouvez ici échanger toutes vos idées</h2><% } %>
    </header>
     <p>Nom du Groupe : <%= nom %><br>
      Description : <%= desc %></p>
    <p>Veuillez choixir le mode d'apprentissage<br>
      que vous voulez pratiquer.</p>
      <%if(iduser==coor)
    	  {%>
     <p><strong><a href="ajoutamis.jsp">Ajouter des amis à votre groupe</a></strong></p>
          <%} %>
    <ul class="actions">
    <li><form method="post" action="DisplayProbIndiv" >
    <input type="hidden" name="idgrp" value=<%=request.getParameter("idgrp") %>>
    <input type="hidden" name="co" value=<%=request.getParameter("co") %>>
